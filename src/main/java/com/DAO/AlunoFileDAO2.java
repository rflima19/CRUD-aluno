...
public class AlunoFileDAO2 {
	...
	public static final File FILE_BINARY = new File(AlunoFileDAO2.DIRECTORY, "alunos.bin");
	public static final File FILE_TXT = new File(AlunoFileDAO2.DIRECTORY, "alunos.txt");
	...
	public synchronized boolean salvarAlunoFileBinario(Aluno aluno) throws SistemaEscolarException {
		if (AlunoFileDAO2.FILE_BINARY.exists() == false) {
			try {
				if (AlunoFileDAO2.DIRECTORY.exists() == false) {
					AlunoFileDAO2.DIRECTORY.mkdir();
				}
				AlunoFileDAO2.FILE_BINARY.createNewFile();
			} catch (IOException e) {
				throw new SistemaEscolarException("Não foi possivel criar o arquivo binário", e);
			}
		}

		try (OutputStream outBytes = new FileOutputStream(AlunoFileDAO2.FILE_BINARY, true);
			...
			throw new SistemaEscolarException(
				"Falha ao abrir o arquivo " + AlunoFileDAO2.FILE_BINARY.getAbsolutePath() + 
					" para escrita", e);
		}
		return true;
	}
	...
	public synchronized boolean salvarAlunoFileTexto(Aluno aluno) throws SistemaEscolarException {
		if (AlunoFileDAO2.FILE_TXT.exists() == false) {
			try {
				if (AlunoFileDAO2.DIRECTORY.exists() == false) {
					AlunoFileDAO2.DIRECTORY.mkdir();
				}
				AlunoFileDAO2.FILE_TXT.createNewFile();
			} catch (IOException e) {
				throw new SistemaEscolarException("Não foi possivel criar o arquivo binário", e);
			}
		}

		try (Writer writer = new FileWriter(AlunoFileDAO2.FILE_TXT, true);
				PrintWriter pw = new PrintWriter(writer)) {
			pw.print(aluno.getMatricula());
			pw.print(AlunoFileDAO2.SEPARADOR);
	...
	private List<Aluno> recuperarRegistrosFileBinario() throws SistemaEscolarException {
		...
		long tamanhoArquivo = AlunoFileDAO2.FILE_BINARY.length();
		long numRegistros = tamanhoArquivo / AlunoFileDAO2.BYTES_REGISTRO;
		try (InputStream inBytes = new FileInputStream(AlunoFileDAO2.FILE_BINARY);
	...
	private List<Aluno> recuperarRegistrosFileTexto() throws SistemaEscolarException {
		...
		try (Reader reader = new FileReader(AlunoFileDAO2.FILE_TXT);
				BufferedReader buffer = new BufferedReader(reader)) {
			String line = null;
			while ((line = buffer.readLine()) != null) {
				String[] tokens = line.split(AlunoFileDAO2.SEPARADOR);
	...
	private boolean persistirRegistrosFileBinario(List<Aluno> alunos) throws SistemaEscolarException {
		if ((alunos == null) || (AlunoFileDAO2.FILE_BINARY.length() == 0)) {
			return false;
		}
		if (AlunoFileDAO2.FILE_BINARY.exists() == true) {
			AlunoFileDAO2.FILE_BINARY.delete();
		}
		for (Aluno aluno : alunos) {
